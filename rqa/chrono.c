...
#include <time.h>
  ...
  void chrono_reportTime( chronometer_t *chrono ) {
    
    printf("%lld,",chrono->xtotal_ns/chrono->xn_events );
  }

  void chrono_report_TimeInLoop( chronometer_t *chrono, int loop_count ) {
    
    printf("%lld,",chrono->xtotal_ns/(chrono->xn_events*loop_count) );
  }

  void chrono_reportTimeDetailed( chronometer_t *chrono ) {
    
    printf("\n deltaT(ns): %lld ns for %ld ops \n"
                                              "        ==> each op takes %lld ns\n",
                  chrono->xtotal_ns, chrono->xn_events, 
                                                chrono->xtotal_ns/chrono->xn_events );
  }

  void chrono_report_TimeInLoopDetailed( chronometer_t *chrono, int loop_count ) {
  ...
  long long chrono_get_Time( chronometer_t *chrono ) {
    
    return chrono->xtotal_ns/chrono->xn_events ;
  }

  long long chrono_get_TimeInLoop( chronometer_t *chrono, int loop_count ) {
    
    return chrono->xtotal_ns/(chrono->xn_events*loop_count) ;
  }
